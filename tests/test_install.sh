...
# Test suite for install.sh subcommands
...
echo "=== install.sh sync test suite ==="
echo ""
...
exit_code=0
bash "$INSTALL_SCRIPT" 2>&1 | tee "$TMPDIR/output1.txt" || exit_code=$?
...
exit_code=0
bash "$INSTALL_SCRIPT" sync 2>&1 | tee "$TMPDIR/output1b.txt" || exit_code=$?

assert "sync-0-args exits non-zero" "[ '$exit_code' -ne 0 ]"
output1b="$(cat "$TMPDIR/output1b.txt")"
assert "sync-0-args prints error about arguments" "echo '$output1b' | grep -qi 'requires exactly two\|usage'"
...
exit_code=0
bash "$INSTALL_SCRIPT" sync onlyname 2>&1 | tee "$TMPDIR/output1c.txt" || exit_code=$?

assert "sync-1-arg exits non-zero" "[ '$exit_code' -ne 0 ]"
output1c="$(cat "$TMPDIR/output1c.txt")"
assert "sync-1-arg prints error about arguments" "echo '$output1c' | grep -qi 'requires exactly two\|usage'"
...
exit_code=0
bash "$INSTALL_SCRIPT" sync a b c 2>&1 | tee "$TMPDIR/output1d.txt" || exit_code=$?

assert "sync-3-args exits non-zero" "[ '$exit_code' -ne 0 ]"
output1d="$(cat "$TMPDIR/output1d.txt")"
assert "sync-3-args prints error about arguments" "echo '$output1d' | grep -qi 'requires exactly two\|usage'"
...
HOME="$MOCK_HOME" AGENTS_SKILLS_DIR="$MOCK_SKILLS" \
  bash "$INSTALL_SCRIPT" sync my-skill "$MOCK_SRC" 2>&1 | tee "$TMPDIR2/output2.txt" || exit_code=$?
...
exit_code=0
HOME="$MOCK_HOME" AGENTS_SKILLS_DIR="$MOCK_SKILLS" \
  bash "$INSTALL_SCRIPT" sync my-skill "$MOCK_SRC" 2>&1 | tee "$TMPDIR2/output3.txt" || exit_code=$?
...
exit_code=0
HOME="$MOCK_HOME" AGENTS_SKILLS_DIR="$MOCK_SKILLS" \
  bash "$INSTALL_SCRIPT" sync my-skill "$MOCK_SRC" 2>&1 | tee "$TMPDIR2/output4.txt" || exit_code=$?
...
exit_code=0
HOME="$MOCK_HOME" AGENTS_SKILLS_DIR="$MOCK_SKILLS" \
  bash "$INSTALL_SCRIPT" sync my-skill "$MOCK_SRC" 2>&1 | tee "$TMPDIR2/output5.txt" || exit_code=$?
...
HOME="$MOCK_HOME3" AGENTS_SKILLS_DIR="$MOCK_SKILLS3" \
  bash "$INSTALL_SCRIPT" sync conflict-skill "$MOCK_SRC3" 2>&1 | tee "$TMPDIR3/output6.txt" || exit_code=$?
...
HOME="$MOCK_HOME4" AGENTS_SKILLS_DIR="$MOCK_SKILLS4" \
  bash "$INSTALL_SCRIPT" sync ghost-skill "$MOCK_SRC_NONEXISTENT" 2>&1 | tee "$TMPDIR4/output7.txt" || exit_code=$?
...
exit_code=0
bash "$INSTALL_SCRIPT" bogus arg1 2>&1 | tee "$TMPDIR/output8.txt" || exit_code=$?
...
# ═══════════════════════════════════════════════════════
echo "=== install.sh unlink test suite ==="
echo ""

# ── Test U0a: unlink with 0 args (after subcommand) ──
echo "Test U0a: unlink with 0 args"
exit_code=0
bash "$INSTALL_SCRIPT" unlink 2>&1 | tee "$TMPDIR/output_u0a.txt" || exit_code=$?

assert "unlink-0-args exits non-zero" "[ '$exit_code' -ne 0 ]"
output_u0a="$(cat "$TMPDIR/output_u0a.txt")"
assert "unlink-0-args prints error about arguments" "echo '$output_u0a' | grep -qi 'requires exactly one\|usage'"
...
exit_code=0
bash "$INSTALL_SCRIPT" unlink a b 2>&1 | tee "$TMPDIR/output_u0b.txt" || exit_code=$?

assert "unlink-2-args exits non-zero" "[ '$exit_code' -ne 0 ]"
output_u0b="$(cat "$TMPDIR/output_u0b.txt")"
assert "unlink-2-args prints error about arguments" "echo '$output_u0b' | grep -qi 'requires exactly one\|usage'"
...
# We override PROJECT_ROOT to TMPDIR5 so the symlink is "owned" by the toolkit.
...
HOME="$MOCK_HOME5" AGENTS_SKILLS_DIR="$MOCK_SKILLS5" PROJECT_ROOT="$TMPDIR5" \
  bash "$INSTALL_SCRIPT" unlink to-remove 2>&1 | tee "$TMPDIR5/output_u1.txt" || exit_code=$?
...
HOME="$MOCK_HOME5" AGENTS_SKILLS_DIR="$MOCK_SKILLS5" \
  bash "$INSTALL_SCRIPT" unlink external-link 2>&1 | tee "$TMPDIR5/output_u2.txt" || exit_code=$?
...
exit_code=0
HOME="$MOCK_HOME5" AGENTS_SKILLS_DIR="$MOCK_SKILLS5" \
  bash "$INSTALL_SCRIPT" unlink nothing-here 2>&1 | tee "$TMPDIR5/output_u3.txt" || exit_code=$?
...
exit_code=0
HOME="$MOCK_HOME5" AGENTS_SKILLS_DIR="$MOCK_SKILLS5" \
  bash "$INSTALL_SCRIPT" unlink my-real-dir 2>&1 | tee "$TMPDIR5/output_u4.txt" || exit_code=$?
...
# ═══════════════════════════════════════════════════════
echo "=== install.sh link-principles test suite ==="
echo ""

# ── Test P0a: link-principles with 0 args (after subcommand) ──
echo "Test P0a: link-principles with 0 args"
exit_code=0
bash "$INSTALL_SCRIPT" link-principles 2>&1 | tee "$TMPDIR/output_p0a.txt" || exit_code=$?

assert "link-principles-0-args exits non-zero" "[ '$exit_code' -ne 0 ]"
output_p0a="$(cat "$TMPDIR/output_p0a.txt")"
assert "link-principles-0-args prints error about arguments" "echo '$output_p0a' | grep -qi 'requires exactly one\|usage'"
...
exit_code=0
bash "$INSTALL_SCRIPT" link-principles a b 2>&1 | tee "$TMPDIR/output_p0b.txt" || exit_code=$?

assert "link-principles-2-args exits non-zero" "[ '$exit_code' -ne 0 ]"
output_p0b="$(cat "$TMPDIR/output_p0b.txt")"
assert "link-principles-2-args prints error about arguments" "echo '$output_p0b' | grep -qi 'requires exactly one\|usage'"
...
HOME="$MOCK_HOME6" AGENTS_PRINCIPLES_DIR="$MOCK_PRINCIPLES" \
  bash "$INSTALL_SCRIPT" link-principles "$MOCK_SRC_PRINCIPLES" 2>&1 | tee "$TMPDIR6/output_p1.txt" || exit_code=$?
...
exit_code=0
HOME="$MOCK_HOME6" AGENTS_PRINCIPLES_DIR="$MOCK_PRINCIPLES" \
  bash "$INSTALL_SCRIPT" link-principles "$MOCK_SRC_PRINCIPLES" 2>&1 | tee "$TMPDIR6/output_p2.txt" || exit_code=$?
...
exit_code=0
HOME="$MOCK_HOME6" AGENTS_PRINCIPLES_DIR="$MOCK_PRINCIPLES" \
  bash "$INSTALL_SCRIPT" link-principles "$MOCK_SRC_PRINCIPLES" 2>&1 | tee "$TMPDIR6/output_p3.txt" || exit_code=$?
...
exit_code=0
HOME="$MOCK_HOME6" AGENTS_PRINCIPLES_DIR="$MOCK_PRINCIPLES" \
  bash "$INSTALL_SCRIPT" link-principles "$MOCK_SRC_PRINCIPLES" 2>&1 | tee "$TMPDIR6/output_p3b.txt" || exit_code=$?
...
exit_code=0
HOME="$MOCK_HOME6" AGENTS_PRINCIPLES_DIR="$MOCK_PRINCIPLES" \
  bash "$INSTALL_SCRIPT" link-principles "$MOCK_SRC_PRINCIPLES" 2>&1 | tee "$TMPDIR6/output_p4.txt" || exit_code=$?
...
exit_code=0
HOME="$MOCK_HOME6" AGENTS_PRINCIPLES_DIR="$MOCK_PRINCIPLES" \
  bash "$INSTALL_SCRIPT" link-principles "$MOCK_SRC_PRINCIPLES" 2>&1 | tee "$TMPDIR6/output_p5.txt" || exit_code=$?
...
exit_code=0
HOME="$MOCK_HOME6" AGENTS_PRINCIPLES_DIR="$MOCK_PRINCIPLES" \
  bash "$INSTALL_SCRIPT" link-principles "$MOCK_SRC_NONEXISTENT" 2>&1 | tee "$TMPDIR6/output_p6.txt" || exit_code=$?
