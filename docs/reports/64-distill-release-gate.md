...
The release candidate contains native Distill CLI artifacts for `darwin-arm64`, `darwin-x64`, `linux-arm64`, and `linux-x64`. Linux artifacts use static MUSL targets so they can be cross-built without depending on the target host's glibc. Package, platform selection, fresh install, upgrade, and uninstall checks passed.
